...
assertEnv('env:NODE_ENV=production', 'NODE_ENV', 'production', 'supports env: values with underscores');
...
if (relResult.code !== 0) {
  incFail('resolves project paths relative to the config file directory (generator exited ' + relResult.code + ')', 'stderr: ' + relResult.stderr);
} else {
  ...
  if (output.includes(expected)) {
    ...
    incFail('resolves project paths relative to the config file directory (expected mount not found)', 'Output:\n' + output);
  }
}
